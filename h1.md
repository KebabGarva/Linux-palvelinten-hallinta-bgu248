...
Tämä raportti on tehty Tero Karvisen toteuttaman kurssin Palvelinten hallinta yhteydessä. Tämä raportti kertoo läpikohtaisesti, mitä tein tehtävän h1 eteen. Sisällysluettelon linkkejä painamalla pääset suoraan haluaamaasi kohtaan.

### Missä?

Toteutin tehtäviin liittyvät komennot fyysisessä Windows 11 - ja virtuaalisissa Debian -ympäristöissä hyödyntämällä [Vagrantia](https://developer.hashicorp.com/vagrant). Spesifoin tarkemmin kappaleen alussa, mitkä ovat käytettävän/käytettävien käyttöjärjestelmien tiedot.
...
- Kaikista suurin virhe on valehdella tehneensä testejä plagioinnin ja tekijänoikeuksien rikkomisten lisäksi.
...
$\left(f\left(x\right)=f\left(fx\right)\right)$
...
Suoritan komennon `sudo salt-call --local grains.items', 'sudo salt-call --local grains.item osfinger virtual`, jota on [Tero Karvisen tehtävänannossa](https://terokarvinen.com/2024/configuration-management-2024-spring/) ohjeistettu käyttämään.
...
Näitä flageja on ehdottomasti paljon enemmän, mitä kuvasssa näkyy. Valitsin muutaman flagin selitettäväksi yhdellä lauseella. CX8-lippu antaa ohjeita muistin hallinnoimiseen vertaamisen ja vaihtamisen näkökulmasta. 
