...
Vagrantilla uuden virtuaalikoneen voi luoda yksinkertaisimmillaan katsomalla ensin Vagrant Cloudista haluttu virtuaalikone, minkä jälkeen hyödynnetään `vagrant init ESIMERKKI/KONE` -komentoa. Törmäsin [FreeCodeCampin artikkeliin](https://www.freecodecamp.org/news/create-and-manage-virtual-machines-with-vagrant/), jossa annettiin muistutus siitä, että pitää luoda oma kansio virtuaalikoneelle. Sitä ennen loin kansion kuin `VagrantTesti`, jonne voi  Itse valitsin satunnaisesti [tämän kyseisen virtuaalikoneen](https://app.vagrantup.com/debian/boxes/testing64) käytettäväksi. Voimme tietenkin väitellä siitä, oliko valinta oikeasti täysin sattumaa. Suoritin `vagrant init debian/testing64` -komennon. Hakemistoon navigoimisen sijaan käytin komentoa `vagrant global-status`. Se on minulle yksinkertaisesti helpompaa nähdä kaikki mahdolliset vaihtoehdot yhdellä komennolla, ja valita ID:n perusteella käynnistettävä virtuaalikone

![image](https://github.com/KebabGarva/Linux-palvelinten-hallinta-bgu248/assets/89390996/683ecb7f-cbec-42bc-a939-b8f6769b6f11)

Tätä kautta voin vain yksinkertaisesti kirjoittaa komennon `vagrant up `
